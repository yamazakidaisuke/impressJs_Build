...
(function ( document, window ) {
    ...
    document.addEventListener("keydown", function ( event ) {
        //** ここに0とテンキーの0のコードを追加 **
        if ( event.keyCode == 9 || ( event.keyCode >= 32 && event.keyCode <= 34 ) || (event.keyCode >= 37 && event.keyCode <= 40)  || (event.keyCode >= 48) || (event.keyCode == 96)) {
...
var TimerView = {
    ...
    //タイマー開始
    "start" : function(){
       GVAL.timerid = setInterval('TimerView.view()',1000);
       GVAL.tflg = 1;
       this.view_cmt("tflg_view","開始");
    },
    //タイマー停止
    "stop" : function(){
       clearInterval( GVAL.timerid );
       GVAL.tflg = 0;
       this.view_cmt("tflg_view","停止");
    },
    //秒表示
    "view" : function(){
        var imin =0;
        var i = GVAL.sec++;
        if(i>=60){
            imin  = Math.floor(i/60);
            i     = Math.floor(i%60);
        }
        var str = imin+"分" + i + "秒"; 
        this.view_cmt("timer_view", str);
    },
    //タイマーリセット
    "clear" : function(){
        clearInterval( GVAL.timerid );
        ...
        GVAL.timerid="";
        GVAL.tflg = 0;
        this.view_cmt("timer_view",0);
    },
    //表示
    "view_cmt":function (el,str){
